...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 10 03:34:33 2020
...
while max(abs(x_true-x_sol))>=0.01:   #  tolerance checking
    ...
    for i in range(len(b)):
        for j in range(len(b)):
            if j!=i:
               sum[i]=sum[i]+A[i][j]*x_sol[j]     
        # relaxation formula (reduced from x(i)=x(i-1)+r(ii)/A(ii))
        x_sol[i]=(1-w)*x_sol[i]+w*((-sum[i]+b[i])/A[i][i])
    count=count+1
print("No of iteration taken by Relaxation Method to reach this accuracy: ",count)
