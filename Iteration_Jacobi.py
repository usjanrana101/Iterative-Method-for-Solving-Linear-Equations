...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  9 21:09:14 2020
...
while max(abs(x_true-x_sol))>=0.01:   #  tolerance checking
    ...
    for i in range(len(b)):
        for j in range(len(b)):
            if j!=i:
               sum[i]=sum[i]+A[i][j]*x_sol[j]     # Jacobi formula          
    for i in range(len(b)):                       # updating the solution
        x_sol[i]=(-sum[i]+b[i])/A[i][i]
    count=count+1
print("No of iteration taken by Jacobi Method to reach this accuracy: ",count)
